...
class ext_tool {
	...
	public static function vaild_scheme( &$data , $scheme ){
		foreach ($scheme as $v) {
			...
			$error = false;
			switch ($valid) {
				case 'url': self::is_url($info) ? '' : $error = true; break;
				case 'email': self::is_email($info) ? '' : $error = true; break;
				case 'int': is_numeric($info) ? '' : $error = true; break;
				case 'array' : is_array($info) ? '' : $error = true; break;
				default: break;
			}
