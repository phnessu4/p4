...
class p4 {
    ...
    protected function __construct($inifile = null, $config_file = null) {
        ...
        if (! file_exists ( $inifile ) || ! file_exists ( $config_file )) {throw new Exception ( '缺少路由文件,模板配置文件' );}
        /* 获取配置文件 */
    ...
    protected function invoke($path, $rules) {
        ...
        if (empty ( $rules ['controller'] )) {throw new Exception ( "默认配置缺少 controller , 错误地址 : " . URL, E_USER_ERROR );}
        if (empty ( $rules ['method'] )) {throw new Exception ( "默认配置缺少 method , 错误地址 : " . URL, E_USER_ERROR );}
        /* 默认controller */
        ...
        if (class_exists ( $class ) == false) {throw new Exception ( "调用未定义方法,请检查配置或定义方法 : $class", E_USER_ERROR );}
